<template>
  <div>
    <v-form class="grey rounded-t-xl pb-1">
      <v-row>
        <v-col sm='2' lg='2'>
          <v-btn class="mx-auto d-block" @click.prevent='visible = !visible'>{{visible ? 'Hide' : 'Show'}} form</v-btn>   
        </v-col>
        <v-col sm="8" lg='8'>
      ...
      <div v-if="visible">
        <v-row class="px-2">
          ...
          <v-col v-for="(point, i) in addedGoal.pointsCount" :key='i + "addedPoint"' style="min-width: 200px">
            <v-text-field
